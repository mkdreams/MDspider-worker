...
function ReplaceNowUser(tab, info, cb) {
	xhrPost(window.spiderSlaveHelpmateApi,{
		...
		params:[[window.workCreateFlag,window.userDataPath,window.spiderSlaveApiCb+'?&isReplace=1&sFlag='+window.spiderSlaveFlag+'&workCreateFlag='+window.workCreateFlag]]
	},undefined,'json').then(function(data) {
		if(cb) {
			textToBase64(JSON.stringify(data),function(base64){
				cb(base64);
